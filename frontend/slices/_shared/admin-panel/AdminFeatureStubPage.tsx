...
export function AdminFeatureStubPage({ segment }: { segment: string }) {
  ...
  if (!block) notFound();
  // PUBLIC-demo isolation: render the bare in-memory block view (useDefault*Bindings)
  // so demo visitors never read/write the shared Convex admin tables. Pure no-op when
  // NEXT_PUBLIC_DEMO !== "1" — the *BlockConvex path below stays the default.
  if (IS_DEMO) {
