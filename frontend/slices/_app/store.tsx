...
// Provider branch: DEMO runs on localStorage (no Convex queries mount); real
// clones run the original Convex-backed provider verbatim. IS_DEMO is a
// build-time const, so the DemoProvider tree-shakes out of real clones.
...
// BroadcastChannel. Mounted only when NEXT_PUBLIC_DEMO=1; never reached in real
// clones (IS_DEMO is a build-time const false).
function DemoProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = React.useMemo<Ctx>(
    // ready=true immediately + progress=100 — the demo state is in hand
    // synchronously, no network load phase to report.
