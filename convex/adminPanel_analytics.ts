...
// read-only by nature. KPIs + the 30-day series are computed from REAL kreator
// tables (kreatorContents, kreatorOrders, subscribers, kreatorComments) via
...
export const get = query({
  ...
  handler: async (ctx) => {
    ...
    const [contents, orders, subscribers, comments] = await Promise.all([
      ctx.db.query("kreatorContents").collect(),
      ctx.db.query("kreatorOrders").collect(),
      ctx.db.query("subscribers").collect(),
      ctx.db.query("kreatorComments").collect(),
    ]);

    // REAL reach proxy: published content carries an explicit `views` counter.
    ...
    const contentCurr = inWindow(contents, windowStart, now);
    const contentPrev = inWindow(contents, prevStart, windowStart);
    ...
    const kpis = [
      {
        ...
        hint: "all published content",
      },
      {
        id: "content",
        label: "Content pieces",
        value: contents.length.toLocaleString(),
        deltaPct: Number(pct(contentCurr, contentPrev).toFixed(1)),
    ...
    // views = content+subscribers+comments+orders created that day (a real
    // activity signal), sessions = subscribers+orders that day.
    const dayKey = (t: number) => new Date(t).toISOString().slice(0, 10);
    const viewsByDay = new Map<string, number>();
    const sessByDay = new Map<string, number>();
    const bump = (m: Map<string, number>, k: string) => m.set(k, (m.get(k) ?? 0) + 1);
    for (const r of [...contents, ...subscribers, ...comments, ...orders]) {
      if (r._creationTime < windowStart) continue;
      bump(viewsByDay, dayKey(r._creationTime));
    }
    for (const r of [...subscribers, ...orders]) {
      if (r._creationTime < windowStart) continue;
    ...
    // --- Traffic sources ---
    // ponytail: illustrative — no event-tracking infra. We DO have a `source`
    // string on subscribers, so the donut is partly real: bucket subscribers
    // into the known palette ids (fallback -> "direct"); other buckets stay 0.
    ...
    // --- Top pages: REAL — published content ranked by its views counter. ---
    const topPages = [...contents]
      ...
      .map((c) => ({
        ...
        title: c.title,
        views: c.views ?? 0,
        // ponytail: illustrative — no event-tracking infra. duration/bounce
        // derived deterministically from likes so the table isn't empty.
        avgDurationSec: Math.max(30, (c.likes ?? 0) + 30),
        bounceRate: 0.35,
      }));

    // --- Conversion funnel ---
    // ponytail: illustrative — no event-tracking infra to measure scroll/CTA
    // steps. Anchored on REAL endpoints: top = total content views, bottom =
    // real orders count; the two middle steps are interpolated drop-offs.
    const visits = Math.max(totalViews, orders.length * 4, 1);
    const submits = orders.length;
    const funnel = [
